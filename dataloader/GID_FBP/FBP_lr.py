...
class FBP_L2H(IterableDataset, BaseDataset):
    def __init__(self, image_list, label_hr_list, label_lr_list,
            ...
            with rasterio.open(image_path) as src_img, \
                ...
                if not self.stride:
                    for i in range(self.num_chips_per_tile):
                        ...
                        if (label_hr_patch == 0).float().mean() > 0.9:
                            continue
                    ...
                    for x in x_list:
                        for y in y_list:
                            ...
                            label_hr_patch = self.label_trans(label_hr)
                            label_lr_patch = self.label_trans(label_lr)
...
def test_5cls():
    ...
    label_hr_list = [fn.replace('.tiff', '.tif')
                  .replace('Image_16bit_BGRNir', 'Annotation_5cls')
                  for fn in image_list]
    label_lr_list = [fn.replace('.tiff', '.tif')
                  .replace('Image_16bit_BGRNir', 'Annotation_24cls_x30')
                  for fn in image_list]
    ds = FBP_L2H(image_list, label_hr_list,label_lr_list, num_chips_per_tile=96)
    dl = DataLoader(ds, batch_size=32, num_workers=3, pin_memory=True)
    for img, label_hr,label_lr in tqdm(dl):
        print(img.shape, label_hr.shape)
        print(np.unique(label_hr.numpy()))
        print(np.unique(label_lr.numpy()))
        break
        pass
