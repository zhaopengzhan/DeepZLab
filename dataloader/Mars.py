...
class Mars(IterableDataset, BaseDataset):
    def __init__(self, image_list, label_list=None, chip_size=14, num_chips_per_tile=50, stride=None):
        super().__init__()

        if label_list is None:
            self.fns = [(image_path, None) for image_path in image_list]
        else:
    ...
    def __iter__(self):
        ...
        for image_path, label_path in self.fns[lower_idx: upper_idx]:
            ...
            if label_path is not None:
                ...
                label = np.zeros(shape=image.shape[-2:])
