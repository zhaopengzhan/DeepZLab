...
class GLC_L2H(IterableDataset, BaseDataset):
    def __init__(self, image_list, label_lr_list,  label_type, chip_size=224, num_chips_per_tile=4,
                 stride=None):
        '''
        训练的时候不需要HR标签，也给不了HR标签
        ...
        '''
        super().__init__()
        self.fns = list(zip(image_list, label_lr_list))
    ...
    def __iter__(self):
        ...
        for image_path, label_lr_path in self.fns[lower_idx: upper_idx]:
            ...
            try:
                # print(os.path.exists(label_lr_path))
                label_lr_fp = rasterio.open(label_lr_path, "r")
            except:
                print(image_path, label_lr_path)
                continue
            # data
            image = image_fp.read()
