...
class SHLU(IterableDataset, BaseDataset):
    def __init__(self, image_list, build_list, landuse_list, chip_size=224, num_chips_per_tile=50, stride=None):
        super().__init__()
        self.fns = list(zip(image_list, build_list, landuse_list))
    ...
    def __iter__(self):
        ...
        for image_path, build_path, landuse_path in self.fns[lower_idx: upper_idx]:
            # data
            image = rasterio.open(image_path, "r").read()
            # image = rasterio.open(image_path, "r").read([1,2,3])
            label_build = rasterio.open(build_path, "r").read()
            label_LU = rasterio.open(landuse_path, "r").read()

            # To Tensor
            image = self.image_trans1(image)
            label_build = self.label_trans(label_build)
            ...
            if not self.stride:
                ...
                for i in range(self.num_chips_per_tile):
                    ...
                    label_build_patch = label_build[..., y:y + self.chip_size, x:x + self.chip_size]
                    label_LU_patch = label_LU[..., y:y + self.chip_size, x:x + self.chip_size]

                    # if (label_LU_patch == 0).float().mean() > (1 - 1e-7):
                    #     continue

                    yield image_patch, label_LU_patch.squeeze(), label_build_patch.squeeze()
            else:
                ...
                for x in x_list:
                    for y in y_list:
                        ...
                        yield image_patch, label_LU_patch.squeeze(), label_build_patch.squeeze(), np.array((y, x))
...
if __name__ == '__main__':
    ...
    ds = SHLU(image_list, build_list, landuse_list)
    dl = DataLoader(ds, batch_size=32, num_workers=4, pin_memory=True)
    for img, build, LU in tqdm(dl):
        print(img.shape, build.shape, LU.shape)
        print(np.unique(build.numpy()))
