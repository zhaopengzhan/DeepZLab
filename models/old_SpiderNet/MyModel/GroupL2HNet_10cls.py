...
from ..BEIT.modeling_finetune import beit_base_patch16_224, VisionTransformer
from ..Paraformer.vit_seg_modeling import DecoderCup, SegmentationHead
from ..Paraformer.vit_seg_modeling_L2HNet import L2HNet, StdConv2d
...
class MyModel(nn.Module):
    ...
    def forward(self, x, *, return_feat=False, return_attn=False, as_dict=False):
        # x [bs, 4+6, 224, 224]
        last_hidden_cnn, cnn_feats, downscale_feat,x_list = self.cnn(x)
        last_hidden_trans, trans_feats, attn_dict_list,attn_list = self.trans(x[:, :3], cnn_feats)
        # for f in trans_feats:
        #     print(f.shape)
        pred_cnn, pred_fusion, pred_trans = self.out(last_hidden_trans, downscale_feat, attn_dict_list)

        # return pred_cnn, pred_fusion, pred_trans, last_hidden_trans, trans_feats, attn_dict_list,attn_list,x_list
        # return pred_cnn, pred_fusion, pred_trans
        return pred_cnn, pred_fusion

    def _init_weight(self, num_classes=8):
        ckt_beit = torch.load(r'F:\Projects\MyGroupVIT6\weights\beit_base_patch16_224_pt22k_ft22k.pth',weights_only=False)['model']
