...
def build_hr_model(config=None, ckpt_pretrained=None,in_channel=3, num_classes=3):

    model = HighResulutionNet(in_channel=in_channel, num_classes=num_classes, cfg=config)
...
if __name__ == '__main__':
    ...
    model = build_hr_model(config=r'F:\Projects2\SpiderNet\models\HRNet\config.yml').to(device)
    y = model(x)
    for i in y:
        print(i.shape)
