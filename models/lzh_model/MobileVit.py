...
class MobileViT(nn.Module):
    def __init__(self, image_size, in_channels, dims=[96, 120, 144], \
...
class MobileViT_Seg(nn.Module):
    def __init__(self, img_size, in_channel, num_classes):
        super(MobileViT_Seg, self).__init__()
        self.backbone = MobileViT(image_size=img_size, in_channels=in_channel)
    ...
    def forward(self, x):
        x1, x2, x3, x4 = self.backbone(x) #16, 64, 80, 384
...
if __name__ == '__main__':
    ...
    model = MobileViT_Seg(img_size=(256,256), in_channel=4, num_classes=7).cuda()
