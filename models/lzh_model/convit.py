...
class Convit_seg(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000, **kwargs):
        super().__init__()
        num_heads = 16
        embed_dim = 48 * num_heads
        self.num_patches = 14
        self.backbone = VisionTransformer(img_size=224, patch_size=16, in_chans=in_chans, num_classes=1000, embed_dim=embed_dim, depth=12,
                 num_heads=num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        for key in checkpoint:
            if "head" in key or "patch_embed" in key or "cls_token" in key:
    ...
    def forward(self, x):
        ...
        if self.backbone.use_pos_embed:
        ...
        for blk in self.backbone.blocks:
            x = blk(x)
            x_list.append(x)
        x_out = torch.stack(x_list, dim=-1).sum(-1)
        x_out = x.permute(0, 2, 1).reshape(B, -1, self.num_patches, self.num_patches)
        x_out = self.final_convs(x_out)
        x_out = F.interpolate(x_out, size=(H, W), mode='bilinear', align_corners=True)
        return x_out
    
if __name__ == '__main__':
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    input = torch.autograd.Variable(torch.randn(2, 4, 224, 224)).to(device)
    net = Convit_seg(in_chans=4, num_classes=17).to(device)
