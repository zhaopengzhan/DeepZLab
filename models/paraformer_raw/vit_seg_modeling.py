...
class Embeddings(nn.Module):
    ...
    def forward(self, x):
        x, features, cnn_feats = self.hybrid_model(x)
        ...
        return embeddings, features, cnn_feats
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        # 把CNN特征编码为token
        embedding_output, features, cnn_feats = self.embeddings(input_ids)
        encoded, attn_weights = self.encoder(embedding_output)  # (B, n_patch, hidden)
        return encoded, attn_weights, features, cnn_feats
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        if x.size()[1] == 1:
            x = x.repeat(1, 3, 1, 1)
        x1, attn_weights, features, cnn_feats = self.transformer(x)  # (B, n_patch, hidden)
        x1, x2 = self.decoder(x1, features)  # x1,
        logits1 = self.segmentation_head1(x1)
        logits2 = self.segmentation_head2(x2)
        # return logits1, logits2, features, cnn_feats
