...
class RPBlock(nn.Module):
    def __init__(self, in_channels, ratios=(1, 0.25, 0.125)):
        ...
        self.branchB = nn.Sequential(
            ...
            nn.GroupNorm(in_channelB // 8, in_channelB),
            nn.GELU()
        )
        ...
        self.branchC = nn.Sequential(
            ...
            nn.Conv2d(in_channelC, in_channelC, (1, 3), padding=(0, 3 // 2)),
            nn.Conv2d(in_channelC, in_channelC, (3, 1), padding=(3 // 2, 0)),
            nn.GroupNorm(in_channelC // 8, in_channelC),
            nn.GELU()
        )
...
class L2HNet(nn.Module):
    def __init__(self,
        ...
        self.width = width
        self.startconv = nn.Conv2d(in_channels, self.width, kernel_size=3, stride=1, padding=1)
...
class L2HNetV2(nn.Module):
    def __init__(self, in_chan, num_classes, width=64):
        super().__init__()
        self.cnn = L2HNet(width=width, image_band=in_chan)
