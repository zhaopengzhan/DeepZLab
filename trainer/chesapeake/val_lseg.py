...
class TestDataset(Dataset, BaseDataset):
    ...
    def __getitem__(self, item):
        ...
        # image_patch = rearrange(image_patch, 'c h w -> h w c')
        ...
        image_ts = self.image_trans2(image_patch)
...
def val_one_epoch(args, model, epoch):
    ...
    # image_list = glob.glob(rf'{args.val_dir}\m_4107506_se_18_1_naip-new.tif')
    image_list = [r'F:\datasets\Remote Sensing\cvpr_chesapeake_landcover\ny_1m_2013_extended-debuffered-train_tiles\m_4107507_ne_18_1_naip-new.tif']
    # 每次训练前先清空一下内存
    gc.collect()
    ...
    for idx, image_path in enumerate(pbar):
        ...
        output = np.zeros((args.num_class, input_height, input_width), dtype=np.float32)
        ...
        for i, (image, coords) in enumerate(dataloader):
            ...
            label_set = [
                ...
                # 'Deciduous Forest', 'nodata', 'nodata',
                # 5 7
            ...
            with torch.no_grad():
                logist = model(image[:, :3, :, :],label_set).cpu().numpy()
