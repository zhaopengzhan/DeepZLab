...
def train_one_epoch(args, train_dl, model, optimizer, epoch, writer, mode='train'):
    ...
    for batch_idx, (image_patch, label_lr_patch, label_hr_patch) in enumerate(pbar):
        ...
        # run
        if mode == 'train':
            pred_cnn, pred_fusion, *_ = model(image_patch)
        else:
            with torch.no_grad():
                pred_cnn, pred_fusion, *_ = model(image_patch)
        ...
        if mode == 'train':
            loss_lr = cross_entropy_loss(pred_cnn, label_lr_patch.to(torch.long))
            loss_plat = cross_entropy_loss(pred_fusion, mask_plat.to(torch.long))

            loss_sum = loss_lr + loss_plat
        ...
        # 计算评价指标
        metric_record = calculate_metrics(pred_cnn, pred_fusion, mask_plat, label_lr_patch, label_hr_patch,
                                          metric_record, args)
