...
@torch.no_grad()
def val_one_epoch(args, model,test_dl, epoch):
    ...
    for idx, image_path in enumerate(pbar):
        ...
        for i, (image, coords) in enumerate(dataloader):
            ...
            with torch.no_grad():
                pred_cnn, pred_trans, *_ = model(image)
            logist = (pred_cnn + pred_trans).cpu().numpy()
...
if __name__ == '__main__':
    ...
    # args.fig_path = r'F:\Projects2\SpiderNet\run\figures\SpiderNet_Chesapeake'
    args.fig_path = r'F:\Projects\MyGroupVIT6\run\visualize\my_model_full_cross'
    args.image_size = 224
    args.num_class = 17
    args.stride = 112
    args.batch_size = 16
    ...
    model = models.build_mymodel2(6, args.num_class)
    checkpoint = torch.load(r'', weights_only=False)
    model.load_state_dict(checkpoint['state_dict'])

    mIoU = val_one_epoch(args, model, epoch=-1)
    print(f'mIoU:{mIoU}')
