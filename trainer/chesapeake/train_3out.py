...
def train_one_epoch(args, train_dl, model, optimizer, epoch, writer, loss_wrapper, mode='train'):
    ...
    if mode == 'train':
        ...
        loss_record.register('loss_auxiliary', AverageMeter())
    ...
    for batch_idx, (image_patch, label_lr_patch, label_hr_patch) in enumerate(pbar):
        ...
        # run
        if mode == 'train':
            pred_cnn, pred_fusion, pred_trans= model(image_patch)
        else:
            with torch.no_grad():
                pred_cnn, pred_fusion, pred_trans = model(image_patch)

        # get intersection mask
        mask_cnn = pred_cnn.argmax(axis=1)
        mask_plat = torch.where(mask_cnn == label_lr_patch, label_lr_patch, 0)

        if mode == 'train':
            loss_lr = loss_wrapper['ce_loss'](pred_cnn, label_lr_patch.to(torch.long))
            loss_plat = loss_wrapper['ce_loss'](pred_fusion, mask_plat.to(torch.long))
            loss_auxiliary = loss_wrapper['ce_loss'](pred_trans, label_lr_patch.to(torch.long))

            loss_sum = loss_lr + loss_plat + 0.05 * loss_auxiliary
            ...
            # loss record
            loss_record['loss_sum'].update(loss_sum, args.batch_size)
            loss_record['loss_lr'].update(loss_lr, args.batch_size)
            loss_record['loss_trans'].update(loss_plat, args.batch_size)
            loss_record['loss_auxiliary'].update(loss_auxiliary, args.batch_size)

        label_lr_patch = label_lr_patch.cpu()
        label_hr_patch = label_hr_patch.cpu()

        # 计算评价指标
        metric_record = calculate_metrics(pred_cnn, pred_fusion, pred_trans, mask_plat, label_lr_patch, label_hr_patch,
                                          metric_record, args)
...
def calculate_metrics(
    ...
    # 计算融合模型的指标
    mask_merge = (pred_fusion + pred_cnn).argmax(axis=1).cpu().detach().numpy()
